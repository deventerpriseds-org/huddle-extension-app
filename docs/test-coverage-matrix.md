...
| Process response — **blocker** → BLOCKED + honest why | 🔨 | **ROOT CAUSE FOUND + FIX DEPLOYED (verifying).** Cause: agents pass a NON-uuid task_id to flag_blocker (a title "Update on backlog grooming" / slug "task-msan556s-z56sy" — seen live in `task_blockers`). Old order wrote the reason row FIRST then journey `update_task{BLOCKED}` which 0-row-failed on the bad id → orphan blocker row + agent claimed blocked while journey/mirror stayed UP_NEXT (false positive). Confirmed write is fine with a valid uuid (repro: liam a596926e → BLOCKED). FIX: (1) journey execute-tool `updateTask` .maybeSingle()+"No task matched id" (PR #25 merged/deployed); (2) huddle flag_blocker BOTH paths — journey BLOCKED write FIRST, gate the reason-row on it, ok:false+honest-report on failure; (3) confirm-reply directive hands the agent the EXACT uuid for flag_blocker. huddle 83c0097 deployed (deploy-swa 31436919167). Live lifecycle verify in progress. |
| Reach-out to unblock + user unblock (immediate/delayed) | 🔨 | verifying: surfaceBlocked DM reach-out + whether chat-driven unblock is wired (unblock may be board/status-driven via clearTaskBlocker only). |
