...
| Process response — **blocker** → BLOCKED + honest why | ⚠️ | **DEFECT FOUND.** flag_blocker DID fire at confirm-reply time, but journey `update_task{status:BLOCKED}` returned "Cannot coerce the result to a single JSON object" → status stayed UP_NEXT, no `task_blockers` row, yet agent's NL reply claimed "flagged as blocked" (trust gap). Cause NOT pinned (enum HAS BLOCKED; setTaskBlocker is plain PG; Path-A's identical update_task succeeded) — needs 1 instrumented live repro before a fix. Run 31434720034. |
