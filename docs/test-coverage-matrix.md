...
| User unblock — **immediate** | ⚠️ | **2 FINDINGS (not yet fixed).** (a) Reply in the COORDINATOR DM (dm-terry, where the reach-out lands) can't unblock — ownership guard defers ("assigned to finn"), and nothing auto-routes it to the owner. (b) If the blocked task still carries `confirm_status='asked'`, the unblock reply is HIJACKED by the confirm-reply directive → agent runs confirm flow and **falsely claims "moved out of Blocked" with NO DB change** — same NL-vs-DB false-positive class as the original bug, on the unblock path. |
