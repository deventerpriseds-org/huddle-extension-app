    ...
    return {
      ...
      note:
        passages.length === 0
          ...
          : `SEMANTIC SEARCH WAS UNAVAILABLE (${semanticError}); these passages come from an exact-word match on "${query}", not a meaning-based one. They are real, but they are whatever happened to contain that word — better passages may exist and were not searched for.`,
    };
  }
