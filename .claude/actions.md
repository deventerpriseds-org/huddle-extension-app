...
- **[OPEN — awaiting user]** Final live confirmation on the user's own mobile that: (a) the meeting Chat tab shows sends+replies, (b) a failed send surfaces a "Couldn't send —" error instead of vanishing, and (c) entering the 1:1 voice meeting makes the mic live+unmuted immediately and the agent hears speech. The mic-gesture reorder is statically correct but only the user's real mobile browser can confirm the activation window now grants the mic. NOTE: 1:1 path only; the GROUP ceremony voice path is ACT-huddle-7 (separate).
