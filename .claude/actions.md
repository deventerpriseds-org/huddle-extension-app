...
**Fix (deployed, 3a44eab):** ArtifactsView already had the responsive skeleton (list full-width on mobile → tap → full-screen preview with a "Back to list" button; AC-2 was already met). The real gap was FILTERING: the folders/status `aside` is `app-hidden md:flex`, so mobile had NO way to filter by lane or status. Added a mobile-only filter bar (`md:app-hidden`) above the list — two horizontally-scrollable chip rows (folders incl. counts + status), driving the same `folder`/`status` state the desktop aside sets. Same component (AC-3). 
**Status:** BUILT + DEPLOYED (tsc+build clean, ff'd to main 3a44eab). Live-confirm pending (phone render).
...
**Fix (BUILT + DEPLOYED, ab5841a — tsc+build clean, ff'd to main):** End-to-end image+file attachments.
- **Upload** — new `uploadChatAttachmentFn` (artifacts/attachments.functions.ts): base64 → `createArtifact` (EXTENDED with an optional initial `status`) into the EXISTING artifact blob store, folder "Uploads", status "approved" (an input, not a review deliverable), `agent_id` = addressed agent so it lands in that agent's Files (ACT-43 scoping). 8MB cap + mime allowlist (images, text/*, pdf, office, .ics). No new storage subsystem — extends the blob/artifacts layer (AC-4 ✓, email-scoped).
- **Turn plumbing** — `Input.attachments` (ids only, ≤6) travels in the payload (bytes never do → durable turn row stays small). Server resolves each id→fresh read SAS once/turn (memoized, shared across responders, fail-safe): **images → OpenAI Responses `input_image` content parts (vision)**; text files (.ics/.txt/.csv/.md/json) → inlined into the message; other binaries → acknowledged by name. Widened `OpenAIPersonaInput.transcript` content to `string | content-parts`; the Lovable path flattens images to a note (its multimodal shape differs). **No-attachment turns are byte-for-byte unchanged** (AC-3 ✓).
- **Composer** (HuddleView.tsx): paperclip button + hidden file input, staged-attachment chips with remove + upload spinner, attachment-only sends allowed (default caption), chips render on the sent user bubble (AC-1, AC-2 ✓).
**Integration trace:** funnels through the ONE turn engine `runHuddleTurn`. Upstream producer = composer `submit()` payload; downstream consumers = the OpenAI Responses transcript (extended) + the Lovable `generateText` messages (flattened) + durable `chat.pending_turns` (ids only, replays fine). Reused: blob store, `getArtifact` (email-scoped resolve, no cross-user SAS leak), artifacts.items (folder "Uploads"). Extended `createArtifact` (+optional status) rather than a parallel store. Distinct from the existing presentational `attachArtifacts` (agent-reply chips) — no collision.
**Status:** BUILT + DEPLOYED. Mechanism verified (tsc+build clean); **NOT yet user-confirmed live** — needs a real upload on the deployed app (screenshot → Iris drafts from it; .ics → parsed). Follow-ons: PDF/docx real parse (input_file), image thumbnail preview on the bubble.

### ACT-huddle-46: Persistent mobile view toggle (Huddle/Board/Files) — nav no longer strands you
**Requested:** 2026-08-11 — "the navigator doesn't persist for toggling between huddle/board/files so when you go to one of the later two you are stuck."
**Root cause (GROUND-TRUTHED):** the Huddle/Board/Files segmented toggle lived ONLY inside HuddleView's header (`HuddleHeader`), which renders only when `view==="huddle"` (HuddleApp: `{view==="huddle"?<HuddleView/>:view==="board"?<BoardView/>:<ArtifactsView/>}`). Leaving the huddle view unmounts the toggle. On DESKTOP the left `Rail` (`app-hidden md:flex`) has the Board/Artifacts switcher, so desktop was fine; on MOBILE the Rail is hidden → switching to Board/Files removed the only switcher and stranded the user.
