...
- **`getTasksForUser` (tasks.server.ts:417) EXCLUDES blocked** — `AND (status IS NULL OR status NOT IN ('DONE','BLOCKED'))`. It backs the `prioritize` tool. A *scheduler* view (blocked ≠ schedulable), but when an agent uses `prioritize` to answer "any blockers?" it returns a **false 'none'** — Elle's live seq-74 answer. **THIS IS THE NEW, UN-FIXED FINDING.**
