...
| Quota surfacing + file-search fix | quota part OK; file-search narration FAILED | PR #4's prose-only house-style ban does not hold — live evidence (iris-chase, finn-reid, cam-post transcripts) shows repeated "...in the uploaded files" narration after this shipped. A prompt rule doesn't reliably override file_search's own trained miss-narration habit. Fixed properly with a deterministic regex backstop (`stripFileMentionNarration`, huddle.functions.ts) mirroring `stripAgentReplyLinks` — don't re-attempt a prose-only fix for this. |
