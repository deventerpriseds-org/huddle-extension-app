...
| routeTurn — ceremony-aware voice barge-in | deployed, independent verifier 6/6 PASS — NOT YET CONFIRMED LIVE by user | commit 864ea0e (main); `routeTurn` in MeetingBar.tsx reads `isCeremonyRef`/`ceremonyStatusRef`/`activeCeremonyTurnRef` via refs (stable useCallback([])), passed as `routeMessage` to groupVoice.start(); useGroupVoice.runTurn() calls it before sendHuddleMessage. Reverted broken f618a04 first (b927f72), then re-implemented correctly this session. |
