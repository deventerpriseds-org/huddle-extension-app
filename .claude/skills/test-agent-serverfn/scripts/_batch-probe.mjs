...
const router = { backend:"openai", model:"gpt-4o-mini", fastMode:false, strictPrompt:false, soloOnCoverage:false, interjections:true, maxInterjectors:6 };
...
for (const r of (val?.replies||[])) { console.log(`\n[${r.agentId}] ${String(r.text||"").replace(/\n/g," ").slice(0,300)}`); }
console.log("\n=== toolUses (top-level) ===");
const tu = val?.toolUses||[];
for (const t of tu) console.log(`  ${t.agentId||t.agent||"?"} → ${t.name||t.tool||t.toolName||JSON.stringify(t).slice(0,80)}`);
console.log("\n=== suggestedTasks ("+(val?.suggestedTasks?.length||0)+") ===");
for (const t of (val?.suggestedTasks||[])) console.log(`  • ${t.title}  [owner:${t.ownerId||"?"}]`);
