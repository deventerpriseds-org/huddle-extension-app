...
const router = { backend:"openai", model:"gpt-4o-mini", fastMode:false, strictPrompt:false, soloOnCoverage:true, interjections:true, maxInterjectors:4 };
...
console.log("HTTP", res.status, "| val keys:", Object.keys(val||{}).join(","));
console.log("decision.reason:", val?.decision?.reason || val?.decisionReason || "(none)");
console.log("\n=== REPLIES ("+(val?.replies?.length||0)+") ===");
for (const r of (val?.replies||[])) {
  console.log(`\n[${r.agentId}]`);
  console.log("  text:", String(r.text||"").replace(/\n/g," ").slice(0,320));
  console.log("  replyKeys:", Object.keys(r).join(","));
  const tu = r.toolUses||r.tools||r.toolCalls||r.tool_calls;
  if (tu) console.log("  tools:", JSON.stringify(tu).slice(0,400));
}
console.log("\n=== suggestedTasks ("+(val?.suggestedTasks?.length||0)+") ===");
for (const t of (val?.suggestedTasks||[])) console.log(`  • ${t.title}  [owner:${t.ownerId||t.assigneeId||"?"} lane:${t.lane||t.status||"?"} date:${t.date||t.dueDate||"-"}]`);
console.log("\n=== journeyTaskUpdates ("+(val?.journeyTaskUpdates?.length||0)+") ===");
