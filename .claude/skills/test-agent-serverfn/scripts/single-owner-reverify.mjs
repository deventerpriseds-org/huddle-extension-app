// Verify SINGLE CAPTURE-OWNER for group turns (main 2bae8eb), board-safe.
// Same 4-lane list, run TWICE (fresh history each), full 15-agent roster.
...
function dump(val){
  ...
  for(const t of tu){ const who=t.agentId??t.authorId??t.author??t.by??"?"; console.log(`    · [${who}] ${t.tool} ok=${t.ok} — ${String(t.summary||"").slice(0,180)}`); }
  const st = val.suggestedTasks||[];
  const filers = [...new Set(st.map(s=>s.ownerId))];
  console.log(`  suggestedTasks (${st.length}) filed by ${filers.length} distinct owner(s): ${filers.join(", ")}`);
...
for(let run=1; run<=2; run++){
  console.log(`\n\n======== RUN ${run} · 4-LANE LIST (single capture-owner) ========`);
