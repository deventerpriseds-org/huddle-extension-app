...
const PUSH: Record<string, Push> = {
  ...
  // NOTE: iris-chase and sam-trent were intentionally REMOVED here. Their rich
  // original prompts ("Daily Itinerary Agent", "Compass Startup Agent") are
  // canonical and were restored (see scripts/revert-assistant-instructions.ts).
  // Per CLAUDE.md, agent prompts are additive-only — do not re-add thinned
  // rewrites here. Role scoping is handled by routing + the shared runtime layer,
  // not by overwriting these prompts.
};
